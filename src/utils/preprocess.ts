...
//   if (!img) return;
//   const image = img.getImageData(0, 0, canvas.width, canvas.height);
...
function getARGB(data: Uint8ClampedArray, i: number) {
  const offset = i * 4;
  return (
    ((data[offset + 3] ?? 0 << 24) & 0xff000000) |
    ((data[offset] ?? 0 << 16) & 0x00ff0000) |
    ((data[offset + 1] ?? 0 << 8) & 0x0000ff00) |
    (data[offset + 2] ?? 0 & 0x000000ff)
  );
}
...
function setPixels(pixels: Uint8ClampedArray, data: Int32Array) {
  ...
  for (let i = 0, al = pixels.length; i < al; i++) {
    offset = i * 4;
    pixels[offset + 0] = (data[i] ?? 0 & 0x00ff0000) >>> 16;
    pixels[offset + 1] = (data[i] ?? 0 & 0x0000ff00) >>> 8;
    pixels[offset + 2] = data[i] ?? 0 & 0x000000ff;
    pixels[offset + 3] = (data[i] ?? 0 & 0xff000000) >>> 24;
  }
}
...
function buildBlurKernel(r: number) {
  ...
  if (blurRadius !== radius) {
    ...
    let bki;
    let bm: Int32Array | undefined;
    let bmi: Int32Array | undefined;
